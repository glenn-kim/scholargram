...
lazy val slickCodeGenTask = (sourceManaged, dependencyClasspath in Compile, runner in Compile, streams) map { (dir, cp, r, s) =>
  val func = FileFunction.cached(s.cacheDirectory / "slick-gen",
    ...
    inFiles.flatMap { sqlPath =>
      ...
      val url = "jdbc:h2:mem:scholargram;MODE=MYSQL;DATABASE_TO_UPPER=FALSE;INIT=runscript from '%s'".format(sqlPath) // connection info for a pre-populated throw-away, in-memory db for this demo, which is freshly initialized on every run
